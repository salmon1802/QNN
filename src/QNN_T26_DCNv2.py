...
class QNN_T26_DCNv2(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="QNN_T26_DCNv2",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=16,
                 num_layers=3,
                 ...
                 **kwargs):
        super(QNN_T26_DCNv2, self).__init__(feature_map,
                                            model_id=model_id,
                                            gpu=gpu,
        ...
        self.crossnet = CrossNetV2(input_dim, num_cross_layers, batch_norm=batch_norm, net_dropout=net_dropout)
    ...
    def forward(self, inputs):
        ...
        y_pred = self.fc(torch.cat([self.qnn(feature_emb), self.crossnet(feature_emb)], dim=-1))
...
class CrossNetV2(nn.Module):
    def __init__(self, input_dim, num_layers, batch_norm=False, net_dropout=0.1):
        super(CrossNetV2, self).__init__()
        self.num_layers = num_layers
        self.cross_layers = nn.ModuleList(nn.Sequential(nn.Linear(input_dim, input_dim),
                                                        nn.BatchNorm1d(input_dim) if batch_norm else nn.Identity(),
                                                        nn.ReLU())
                                          for _ in range(self.num_layers))
        self.dp = nn.Dropout(net_dropout)
...
class QuadraticNeuralNetworks(nn.Module):
    def __init__(self,
        ...
        for i in range(num_layers):
            self.layer.append(QuadraticLayer(input_dim, num_row=num_row, num_heads=num_heads, batch_norm=batch_norm,
                                             num_fields=num_fields, net_dropout=net_dropout))
            if net_dropout > 0:
