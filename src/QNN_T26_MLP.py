...
class QNN_T26_MLP(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="QNN_T26_MLP",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=16,
                 num_layers=3,
                 ...
                 **kwargs):
        super(QNN_T26_MLP, self).__init__(feature_map,
                                      model_id=model_id,
                                      gpu=gpu,
        ...
        self.mlp = MLP_Block(input_dim=feature_map.sum_emb_out_dim(),
                             ...
                             batch_norm=batch_norm)
        self.fc = nn.Linear(input_dim + hidden_units[-1], 1)
    ...
    def forward(self, inputs):
        ...
        y_pred = self.fc(torch.cat([self.qnn(feature_emb), self.mlp(feature_emb)], dim=-1))
