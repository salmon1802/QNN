...
from fuxictr.pytorch.layers import FeatureEmbedding, CompressedInteractionNet, LogisticRegression


class QNN_T26_xDeepFM(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="QNN_T26_xDeepFM",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=16,
                 num_layers=3,
                 ...
                 **kwargs):
        super(QNN_T26_xDeepFM, self).__init__(feature_map,
                                          model_id=model_id,
                                          gpu=gpu,
        ...
        self.qnn = QuadraticNeuralNetworks(input_dim=feature_map.sum_emb_out_dim(),
    ...
    def forward(self, inputs):
        ...
        y_pred = self.qnn(feature_emb.flatten(start_dim=1)) + self.lr_layer(X) + self.cin(feature_emb)
